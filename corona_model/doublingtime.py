...
def main():
    ...
    multi_experiments = {
        "doublingtime_a": {"basemodel": {"World" : [("stateCounterInterval", 1)],}},
    }
    ...
    for (request_name, modelConfigs) in multi_experiments.items():
        ...
        if load_data:
            ...
            for k in reversed(double_data.keys()):
                if k > 20 and True:
                    kd2 = int(k/2)
                    double_data[k] = [abs(i-j) for (i,j) in zip(double_data[kd2], double_data[k])] 
